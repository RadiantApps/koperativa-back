...
const getSelectQuery = (code, vars = "") => {
  const queries = {
    ...
    1003: `SELECT * FROM ${databaseTables.PORTFOLIO};`,
    ...
    1008: `
        ...
        FROM 
            portfolio p
        LEFT JOIN 
            portfolio_item pi ON p.id = pi.portfolio_id
        LEFT JOIN 
            portfolio_content pc ON p.id = pc.portfolio_id
        LEFT JOIN 
            portfolio_deliverables pd ON p.id = pd.portfolio_id
    ...
    1015: `SELECT * FROM ${databaseTables.JOBS}`,
    ...
    1025: `SELECT * FROM ${databaseTables.NEXT_POST} WHERE portfolio_id = ?;`,
  };
  return queries[code];
};
