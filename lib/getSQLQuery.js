...
const getSelectQuery = (code, vars = "") => {
  const queries = {
    ...
    1033: `SELECT 
            ...
            CASE 
                WHEN lw.type = 'work' THEN w.title
                WHEN lw.type = 'jobs' THEN j.title 
                WHEN lw.type = 'news' THEN n.title 
            END AS title,

            CASE 
                WHEN lw.type = 'work' THEN pi.title
                WHEN lw.type = 'jobs' THEN j.job_type 
                WHEN lw.type = 'news' THEN n.created_at 
            END AS subtitle,  

            CASE 
                WHEN lw.type = 'work' THEN COALESCE(w.photo) 
                WHEN lw.type = 'jobs' THEN j.photo 
                WHEN lw.type = 'news' THEN n.photo 
            END AS photo,  

            CASE 
                WHEN lw.type = 'work' THEN COALESCE(w.created_at, pi.created_at) 
                WHEN lw.type = 'jobs' THEN j.created_at 
                WHEN lw.type = 'news' THEN n.created_at 
            END AS created_at

        FROM latest_work lw
        LEFT JOIN ${databaseTables.PORTFOLIO} w 
            ON lw.reference_id = w.id AND lw.type = 'work'
        LEFT JOIN ${databaseTables.JOBS} j 
            ON lw.reference_id = j.id AND lw.type = 'jobs'
        LEFT JOIN ${databaseTables.BLOG} n 
            ON lw.reference_id = n.id AND lw.type = 'news'
        LEFT JOIN ${databaseTables.PORTFOLIOITEM} pi 
            ON lw.reference_id = pi.portfolio_id 
        ORDER BY created_at DESC;
    `,
    1034: `SELECT * FROM ${databaseTables.OURSTORY} ORDER BY \`order\` ASC;`,
  };
  return queries[code];
};
const getInsertQuery = (code, vars) => {
  const queries = {
    ...
    2011: `INSERT INTO ${databaseTables.JOBS} (title, city, job_type, description, photo) VALUES (?, ?, ?, ?, ?);`,
    ...
    2024: `INSERT INTO ${databaseTables.LATESTWORK} (reference_id, type, \`order\`) VALUES (?, ?, ?)`,
    2025: `INSERT INTO ${databaseTables.OURSTORY} (content_type, content_data, \`order\`) VALUES (?, ?, ?);`,
  };
  return queries[code];
};
const getUpdateQuery = (code, vars) => {
  const queries = {
    ...
    3016: `UPDATE ${databaseTables.LATESTWORK} SET \`order\` = ? WHERE id = ?;`,
    3017: `UPDATE ${databaseTables.BLOGDETAILS} SET \`order\` = ? WHERE id = ?;`,
    3018: `UPDATE ${databaseTables.OURSTORY} SET \`order\` = ? WHERE id = ?;`,
    3019: `UPDATE ${databaseTables.OURSTORY}
            SET 
                content_type = COALESCE(?, content_type),
                content_data = COALESCE(?, content_data)
            WHERE 
...
const getDeleteQuery = (code, vars) => {
  const queries = {
    ...
    4021: `DELETE FROM ${databaseTables.LATESTWORK} WHERE id = ?`,
    4022: `DELETE FROM ${databaseTables.OURSTORY} WHERE id = ?`,
    4023: `DELETE FROM ${databaseTables.LATESTWORK} WHERE reference_id = ? `,
  };
  return queries[code];
};
