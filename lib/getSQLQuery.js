...
const getSelectQuery = (code, vars = "") => {
  const queries = {
    ...
    1032: `
        ...
        ORDER BY p.order;
    `,
    1033: `SELECT lw.id, lw.reference_id, lw.type, lw.order,
    ...
    CASE 
        WHEN lw.type = 'work' THEN w.photo 
    ...
    CASE 
        WHEN lw.type = 'work' THEN w.created_at 
        WHEN lw.type = 'jobs' THEN j.created_at 
        WHEN lw.type = 'news' THEN n.created_at 
    END AS created_at

FROM latest_work lw
LEFT JOIN ${databaseTables.PORTFOLIO} w ON lw.reference_id = w.id AND lw.type = 'work'
LEFT JOIN ${databaseTables.JOBS} j ON lw.reference_id = j.id AND lw.type = 'jobs'
LEFT JOIN ${databaseTables.BLOG} n ON lw.reference_id = n.id AND lw.type = 'news'
